...
contract('IdaToken', function(accounts){
    ...
    it('initialized the contract with the current values', function(){
        return IdaToken.deployed().then(function(instance){
            tokenInstance = instance;
            return tokenInstance.name();
        }).then(function(name){
            assert.equal(name, 'Ida Token', 'has the correct name')
            return tokenInstance.symbol()
        }).then(function(symbol){
            assert.equal(symbol, 'IDM', 'has the correct Symbol currency')
            return tokenInstance.standard();
        }).then(function(standard){
            assert.equal(standard, 'Ida Token v1.0', 'has the correct standard')
        })
    })
    ...
    it('Transfers token ownership', function(){
        return IdaToken.deployed().then(function(instance){
            ...
            assert.equal(receipt.logs[0].args._from, accounts[0], 'log the account the tokens are transferred from');
            assert.equal(receipt.logs[0].args._to, accounts[1],  'log the account the tokens are transferred to');
